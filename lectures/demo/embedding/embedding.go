...
func automate(item WarehouseAutomator) {
	...
	fmt.Printf("Ship %v via %v ", item, item.Convey())
}
...
func main() {
	...
	// waste := ToxicWaste{50000}
	// automate(&waste) // Error: cannot use waste (type ToxicWaste) as type WarehouseAutomator in argument to automate:
}
